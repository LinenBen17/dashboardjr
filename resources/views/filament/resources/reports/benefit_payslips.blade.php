    ...
    @foreach ($payrollData['data'] as $item)
        ...
        <div class="container">
            ...
            <table class="datosEmpleado">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{ date_diff(date_create($item['fechaIngreso']), date_create($to))->format('%a') > 365 ? 365 : date_diff(date_create($item['fechaIngreso']), date_create($to))->format('%a') }}
                        </td>
            ...
            <table class="datosMonetarios">
                ...
                <tbody>
                    <tr>
                        <td colspan="2" style="text-align:center;">POR PAGO DE
                            <b>{{ $payrollData['benefits'][$benefit] }}</b>
                        </td>
                        <td style="text-align: right">
                            {{ date_diff(date_create($item['fechaIngreso']), date_create($to))->format('%a') > 365 ? number_format($item['sueldo'] * 2, 2) : number_format(($item['sueldo'] * 2 * date_diff(date_create($item['fechaIngreso']), date_create($to))->format('%a')) / 365, 2) }}
                        </td>
                        <td>&nbsp;</td>
                        <td style="text-align: right">
                            {{ date_diff(date_create($item['fechaIngreso']), date_create($to))->format('%a') > 365 ? number_format($item['sueldo'] * 2, 2) : number_format(($item['sueldo'] * 2 * date_diff(date_create($item['fechaIngreso']), date_create($to))->format('%a')) / 365, 2) }}
                    ...
                    <tr>
                        ...
                        <td style="text-align: center">
                            <b>{{ date_diff(date_create($item['fechaIngreso']), date_create($to))->format('%a') > 365 ? number_format($item['sueldo'] * 2, 2) : number_format(($item['sueldo'] * 2 * date_diff(date_create($item['fechaIngreso']), date_create($to))->format('%a')) / 365, 2) }}</b>
                        </td>
                    </tr>
                </tbody>
            </table>
