...
class ShipmentDelivery extends Page
{
    ...
    {
        if ($this->shipment_entry_guide == '') {
            Notification::make()
                ->title('Error: Debe ingresar una guía de envío.')
                ->danger()
                ->send();
        ...
        $shipment_delivery_exists = ModelsShipmentDelivery::where('shipment_entry_id', $shipment_entry_id->id)->first();

        if ((empty($this->received_by_name) && empty($this->received_by_document))) {
            Notification::make()
                ->title('Error: Debe llenar al menos un campo de quien recibe.')
                ->danger()
                ->send();
            $this->dispatch('restartFocus');
            return;
        ...
        if ($shipment_delivery_exists !== null) {
            ...
            $shipment_entry_id->logSnapshot(
                'delivery_update',
                'Actualización de datos de entrega por ' . $this->user->custom_fields['user_name'] . '. Horario: ' . Carbon::now()->format('d/m/Y H:i:s'),
            );
        ...
        $shipment_entry_id->logSnapshot(
            'delivery_created',
            'Registro de datos de entrega por ' . $this->user->custom_fields['user_name'] . '. Horario: ' . Carbon::now()->format('d/m/Y H:i:s'),
        );
